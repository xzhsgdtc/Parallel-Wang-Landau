...
Monomer::Monomer(const Monomer& a) {
    ...
    *mCoord = *a.mCoord;
    ...
    mID = a.mID;
    mCellID = a.mCellID;
    mNeighborWaters = a.mNeighborWaters;
...
void Monomer::backup(std::ofstream& fout) {
    ...
    fout.write(reinterpret_cast<char*> (&mNeighborWaters), sol);
    mCoord->backup(fout);
}
...
void Monomer::recover(std::ifstream& fin) {
    ...
    fin.read(reinterpret_cast<char*> (&mNeighborWaters), sol);
    if(mCoord != NULL) delete mCoord;
...
void Monomer::print(std::ostream& out) {
    out << "Type  " << mType << "\n"
            ...
            << "Number of Neighbor Waters " << mNeighborWaters 
            << std::endl;
