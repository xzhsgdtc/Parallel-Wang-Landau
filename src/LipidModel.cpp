...
void LipidModel::init(std::string initConfFile){
    ...
    *mOut << "       -> building cells ... \n";
    ...
    long apl = ceil(pow((double)mNumberOfMonomers, 1.0/3));
    long box_index;
    long atom_index(0);
    double x, y, z;
    double xo, yo, zo;
    ...
    *mOut << "       -> adding monomers ... \n"
          << "          * monomers per line " << apl << "\n";
    
    delta = mLengthOfSimBox/(apl+1);
    hdelta = 0.3*(delta/2.0);
    xo = yo = zo = delta/2.0;
    ...
    if(!mInitType){
        for(long k=0; k<apl && atom_index < mNumberOfMonomers; ++k){
            for(long j=0; j<apl && atom_index < mNumberOfMonomers; ++j){
                for(long i=0; i<apl && atom_index < mNumberOfMonomers; ++i){
                    ...
                    x = xo + i*delta + mRandom -> nextDouble(-hdelta, +hdelta);
                    y = yo + j*delta + mRandom -> nextDouble(-hdelta, +hdelta);
                    z = zo + k*delta + mRandom -> nextDouble(-hdelta, +hdelta);
        ...
        *mOut << "          * created monomers " << atom_index << "\n";
        
        *mOut << "       -> building monomer indexes ... \n";
        buildMonomerIndexes();

        *mOut << "       -> creating lipids ... \n";
        ...
        *mOut << "          * initial number of lipids: " << initNumberOfLipids << "\n";
        int l=0;
        ...
        case PRINT_MODEL_INFORMATION :
            {
                ...
                out << std::left
                    ...
                    << std::setw(30) << "Length Box, Cell (Min.) "   << std::setw(15) << mLengthOfSimBox    
                    ...
                    << std::setw(30) << "Move Fraction "             << std::setw(15) << mMoveFraction[0] << " " << std::setw(15) << mMoveFraction[1] << " " << std::setw(15) << mMoveFraction[2]<< "\n"
...
void LipidModel::initBonds(){
    if(mBonds == NULL){
        ...
        for(int i=0; i < mNumberOfMonomers; ++i){
            ...
            for(int j=0; j < MAXBOND; ++j) mBonds[i][j] = -1;
        }
    }else{
        for(int i=0; i < mNumberOfMonomers; ++i)
            for(int j=0; j < MAXBOND; ++j) mBonds[i][j] = -1;
...
void LipidModel::delBond(const long& a, const long& b) {
    if (a > b) {
        ...
        errorMsg("delBond(...)", "cannot bond itself !");
        exit(1);
    }

}

void LipidModel::errorMsg(std::string func, std::string msg){
...
double LipidModel::potentialLJ(const Monomer& m1, const Monomer& m2){
    MathVector<double> a = (*m1.mCoord - *m2.mCoord);
    a.minImage(mLengthOfSimBox);
    double r = a.module();
...
double LipidModel::potential(const Monomer& m, const Monomer& n){
    ...
    switch(m.mType){
        case W:
            switch(n.mType){
                case W: e = potentialLJ(m,n);       break;
                case P: e = potentialLJ(m,n);       break;
                case H: e = potentialSCR(m,n);      break;
            }
            break;
        case H:
            switch(n.mType){
                case W: e = potentialSCR(m,n);       break;
                case H: e = (isBonded(m.mID,n.mID) ? potentialFENE(m,n) : potentialLJ(m,n));  break;
                case P: e = (isBonded(m.mID,n.mID) ? potentialFENE(m,n) : potentialSCR(m,n)); break;
            }
            break;
        case P: 
            switch(n.mType){
                case W: e = potentialLJ(m,n); break;
                case P: e = potentialLJ(m,n); break;
                case H: e = (isBonded(m.mID,n.mID) ? potentialFENE(m,n) : potentialSCR(m,n)); break;
            }
            break;
    }
    return e;
...
double LipidModel::potential(const long& i, const bool& countWaterFlag){
   long mi, mj, mk;
   ...
   MonomerIndex kptr;
   mi = *iptr;
   if(countWaterFlag) mMonomers[mi].mNeighborWaters = 0;
   while( jptr != mCells[ci].mMonomers.end() ){
       if(jptr != iptr){
            ...
            if( countWaterFlag && 
                mMonomers[mi].mType == W && 
                mMonomers[mj].mType == W && 
                canBeBonded(mMonomers[mi], mMonomers[mj]) ){
                
                mMonomers[mi].mNeighborWaters++;
            }
       }
       jptr++;
   ...
   while( c_iter != mCells[ci].mNeighbors.end() ){
       ...
       while( kptr != mCells[ck].mMonomers.end() ){
           ...
           if( countWaterFlag && 
               mMonomers[mi].mType == W && 
               mMonomers[mk].mType == W && 
               canBeBonded(mMonomers[mi], mMonomers[mk]) ){
...
void LipidModel::insertLipid(const long& head, const long& middle, const long& tail){
    if( mMonomers[head].mType != W      ||
        mMonomers[middle].mType != W    ||
        mMonomers[tail].mType != W      ) {
        errorMsg("insertLipid(...)", "cannot created lipid, not all the monomers are W type!");
        exit(1);
    }

    // change the middle
    mEnergy -= potential(middle, true);
    mMonomers[middle].mType = H;
    mEnergy += potential(middle);

    
    // change the tail
    mEnergy -= potential(tail);
    mMonomers[tail].mType = H;
    addBond(middle, tail);
    mEnergy += potential(tail);
    
    // change the head
    mEnergy -= potential(head);
    mMonomers[head].mType = P;
    addBond(head, middle);
    mEnergy += potential(head);
...
bool LipidModel::canBeBonded(const Monomer& m, const Monomer& n){
    MathVector<double> a = *m.mCoord - *n.mCoord;
    a.minImage(mLengthOfSimBox);
    double dis = a.module();
...
void LipidModel::deleteLipid(long i){
    if (i == -1) {
        i = mRandom -> nextLong(1, mNumberOfLipids);
    }
    ...
    mMonomers[ mLastDeletedLipid[0] ].mType = W;
    mEnergy += potential( mLastDeletedLipid[0] );

    // change tail monomer
    mEnergy -= potential( mLastDeletedLipid[2] );
    mMonomers[ mLastDeletedLipid[2] ].mType = W;
    delBond( mLastDeletedLipid[1], mLastDeletedLipid[2]);
    mEnergy += potential( mLastDeletedLipid[2] );

    // change middle monomer
    mEnergy -= potential( mLastDeletedLipid[1] );
    mMonomers[ mLastDeletedLipid[1] ].mType = W;
    mEnergy += potential( mLastDeletedLipid[1], true);
...
void LipidModel::undoReptationMove(){
    ...
    if(mReptationFlag){
        ...
        // change back type
        mMonomers[curr_lipid_head].mType = W;
        mMonomers[mLastDeletedLipid[0]].mType = P;
        mMonomers[mLastDeletedLipid[1]].mType = H;
        mMonomers[mLastDeletedLipid[2]].mType = H;
        ...
        // change back type
        mMonomers[curr_lipid_tail].mType = W;
        mMonomers[mLastDeletedLipid[0]].mType = P;
        mMonomers[mLastDeletedLipid[1]].mType = H;
        mMonomers[mLastDeletedLipid[2]].mType = H;
...
double LipidModel::cFactor() {
    ...
    switch (mMoveProposal) {
        ...
        case INSERTIONMOVE: // creation
            neighborWaters = mMonomers[ mLipidWater[mLastCreatedLipidHeadIDInLipidWater+1] ].mNeighborWaters;
            if(neighborWaters <= 0){
                errorMsg("cFactor()", " INSERTION -- zero neighbor waters !");
                exit(1);
            }
            ratio = (mNumberOfMonomers - 3 * (mNumberOfLipids - 1)) * neighborWaters * (neighborWaters - 1);
            ratio /= (double) mNumberOfLipids;
            break;

        case DELETIONMOVE: // deletion
            neighborWaters = mMonomers[ mLastDeletedLipid[1] ].mNeighborWaters;
            if(neighborWaters <= 0){
                errorMsg("cFactor()", " DELETIONMOVE -- zero neighbor waters !");
                exit(1);
            }
            ratio = mNumberOfLipids + 1;
            ratio /= (double) (mNumberOfMonomers - 3 * (mNumberOfLipids)) * neighborWaters * (neighborWaters - 1);
            break;
...
void LipidModel::checkModelIntegrity(){
    ...
    for(i=0; i<mNumberOfLipids; ++i){
        ...
        ij = ii+1;
        ik = ii+2;
        if(mMonomers[mLipidWater[ii]].mType != P ||
           mMonomers[mLipidWater[ij]].mType != H ||
           mMonomers[mLipidWater[ik]].mType != H){
    ...
    for(i=3*mNumberOfLipids; i< mNumberOfMonomers; ++i){
        if(mMonomers[mLipidWater[i]].mType != W){
...
void LipidModel::unpack(long* longData, double* doubleData){
    ...
    *mOut << "     => Unpack package and rebuild the model ... " << std::endl;
    ...
    if(    longData[1] > mMaxNumberOfLipids 
        || longData[1] < mMinNumberOfLipids
        || doubleData[0] > mMaxEnergy
        || doubleData[0] < mMinEnergy) {
        
        errorMsg("LipidModel::unpack", "replica exceeds the model constraints! Give up unpacking !");
        return;
    }
...
RandomAccessNeighborList LipidModel::findWaterNeighborList(const long& id){
    ...
    while(jptr != mCells[ci].mMonomers.end()){
        if(jptr != iptr && mMonomers[*jptr].mType == W && canBeBonded(mMonomers[*iptr], mMonomers[*jptr])){
    ...
    while( c_kptr != mCells[ci].mNeighbors.end() ){
        ...
        kptr = mCells[ck].mMonomers.begin();
        while(kptr != mCells[ck].mMonomers.end()){
            if(mMonomers[*kptr].mType == W && canBeBonded(mMonomers[*iptr], mMonomers[*kptr])){
