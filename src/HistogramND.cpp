...
double HistogramND::dosAt(const double* observables){
    if(!isInLimit(observables)) return -1;
    return mBins[bIndex(observables)].dos;
}
...
void HistogramND::print(std::ostream& out){
    ...
    for(long i=0; i<mNumberOfBins; ++i){
        ...
        for (long d = 0; d < mDim; ++d) {
            if(mDiscreteFlag[d]){
                out << std::setw(10) << (long)ceil(mBins[i].min[d]-0.5) << "  "
                    << std::setw(10) << (long)ceil(mBins[i].min[d]-0.5) << "  ";
            }else{
