...
const long NOBOND = -1000;              // constant to indicate there is not bond 
...
public:
     ...
     */
    void init(std::string initConfFile="");
     ...
     */
    double potential(const long& index);
...
private:
    ...
    bool mIsMoveValid;              // Indicator of whether a trial move is valid
    ...
    double mLowerBoundOfSimBox;     // lower bound of simulation box
    double mHigherBoundOfSimBox;    // higher bound of simulation box    
    ...
    void errorMsg(std::string func, std::string msg, bool isWarning=false);
    void buildCells();
    ...
    /**
     * Undo the move of inserting a lipid
     */
    ...
    /**
     * Undo the move of deleting a lipid
     */
     ...
     */
    void doExpansionMove();
    void undoExpansionMove();

    /**
     * Perform shrink move for the simulation box
     */
    void doShrinkageMove();
    void undoShrinkageMove();
    ...
    */
    ...
    inline void applyBoundaryCondition(Monomer& a){
        a.mCoord -> imposePBC(mLowerBoundOfSimBox,mHigherBoundOfSimBox);
    }
    ...
    inline long whichCell(const double& x, const double& y, const double& z = 0){
       return mCoord2CellID[(long)floor( (x-mLowerBoundOfSimBox) / mLengthOfCell)]
                           [(long)floor( (y-mLowerBoundOfSimBox) / mLengthOfCell)]
                           [(long)floor( (z-mLowerBoundOfSimBox) / mLengthOfCell)];
    ...
    inline long whichCell(const Monomer& a){
        return mCoord2CellID[(long)floor((a.mCoord->get(0)-mLowerBoundOfSimBox) / mLengthOfCell)]
                            [(long)floor((a.mCoord->get(1)-mLowerBoundOfSimBox) / mLengthOfCell)]
                            [(long)floor((a.mCoord->get(2)-mLowerBoundOfSimBox) / mLengthOfCell)];
    ...
    double FENE_R2;         
    double FENE_ro;         
    double FENE_WCA_cutoff[NUMBEROFMONOMERTYPES][NUMBEROFMONOMERTYPES];
