...
public:
    ...
    double PWL_dosRatio(const double* obs);
...
void WLFrame<ModelType>::collectData(){
    ...
    std::ofstream fout[2];
    getOutputStream(fout[0],mPrefix+"dos.dat");
    getOutputStream(fout[1],mPrefix+"norDos.dat");
...
void WLFrame<ModelType>::init(){
    ...
    *mOut << "  >> initialize model ... " << std::endl;
    ...
    mModel -> init(addPrefix("_InitConf.mol2"));
    ...
    *mOut << "  >> modifying model to fit constraints ... " << std::endl;
    ...
    std::string files[NOF]={  mPrefix+"_ModelInfo.dat",
                              mPrefix+"_CellInfo.dat",
                              mPrefix+"_ConfInfo.mol2",
                              mPrefix+"_HistogramInfo.dat"};
...
bool WLFrame<ModelType>::PWL_proposeReplicaExchange(  int num_procs_per_win, 
    ...
    if(inter_win_comm_id != -1){
        ...
        *mOut << "           * My Observables:      ";
        for(int o=0; o < num_observables; ++o) *mOut << std::setw(8) << observables[o] << ", ";
        *mOut << std::endl;
        ...
        *mOut << "           * Partner Observables: ";
        for(int o=0; o < num_observables; ++o) *mOut << std::setw(8) << observables[o] << ", ";
        *mOut << std::endl;

        myfrac = PWL_dosRatio(observables);
...
template <class ModelType>
double WLFrame<ModelType>::PWL_dosRatio(const double* obs){

    return dosRatio(mModel->observables(),obs);
}
...
void WLFrame<ModelType>::PWL_init(int dim, int* num_wins, int* id, double* overlap){
    ...
    for(int d=0; d<dim; ++d){
        if(mHistogramType[d]){
            step[d] = floor( (mMaxLimits[d]-mMinLimits[d])/(double)mHistogramSize[d]+0.5 );
